#!/usr/bin/env python3
"""BW1100 Phase4 analytic parallel-semantics quick validation.
...
_ROOT = os.path.dirname(os.path.dirname(_TEST))
...
SYS_PATH = os.path.join(_ROOT, 'systems', 'BW1100.json')
...
# Small virtual topologies for the one-card quick suite. They allocate no
# multi-GPU tensors: Calculon checks formulas/events/timing using the calibrated
# network model. This is semantic validation, not a physical collective test.
QUICK_CASES = {
    'quick_tp2': dict(num_procs=8, tensor_par=2, pipeline_par=2,
                      data_par=1, expert_par=2, context_par=1, batch_size=4),
    'quick_ep4': dict(num_procs=8, tensor_par=1, pipeline_par=2,
                      data_par=1, expert_par=4, context_par=1, batch_size=4),
    'quick_pp4': dict(num_procs=8, tensor_par=1, pipeline_par=4,
                      data_par=1, expert_par=2, context_par=1, batch_size=8),
    'quick_cp2': dict(num_procs=16, tensor_par=1, pipeline_par=4,
                      data_par=1, expert_par=2, context_par=2, batch_size=8),
}
...
def main() -> None:
    ...
    p.add_argument('--case', choices=list(CASES) + ['all', 'quick'], default='quick')
    ...
    p.add_argument('-v', '--verbose', action='store_true',
                   help='Keep C++ simulator logs even without --timeline '
                        '(sets SIM_VERBOSE=1)')
    p.add_argument('--formula-only', action='store_true',
                   help='Skip expensive C++ flow; check sizes and analytic '
                        'pipeline bubble only (one-card quick mode)')
    args = p.parse_args()
    ...
    names = (list(QUICK_CASES) if args.case == 'quick' else
             list(CASES) if args.case == 'all' else [args.case])
    rows = []
    failed = 0
    ...
    for name in names:
        ...
        if args.formula_only:
            microbatches = max(1, int(model.exe._num_microbatches))
            pp = int(model.exe.pipeline_par)
            bubble = (pp - 1) / (microbatches + pp - 1)
            print(f'FORMULA PASS: TP={model.exe.tensor_par} '
                  f'EP={model.exe.expert_par} PP={pp} CP={model.exe.context_par} '
                  f'microbatches={microbatches} '
                  f'pipeline_bubble_fraction={bubble:.6f} '
                  f'ep_bytes={model._ep_fw_comm_size} '
                  f'cp_bytes={model._cp_fw_comm_size}')
            rows.append({'name': name, 'global_time': 0.0,
                         'total_comm': 0.0, 'ep_fw': 0.0, 'ep_bw': 0.0,
                         'ep_comm': 0.0, 'cp_fw': 0.0, 'cp_bw': 0.0,
                         'cp_comm': 0.0, 'flow_s': 0.0,
