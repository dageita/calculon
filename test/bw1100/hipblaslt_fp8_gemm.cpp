// Native FP8 GEMM timing backend for BW1100 (gfx938).
// PyTorch's DTK 2.5.1 build does not expose Float8 through torch._scaled_mm,
// therefore the FP8 path intentionally uses the installed hipBLASLt directly.
...
int main(int argc, char** argv) {
  ...
  HIP_CHECK(hipMalloc(&d, size_t(m) * n * sizeof(__half)));
  ...
  LT_CHECK(hipblasLtMatrixLayoutCreate(&A, HIP_R_8F_E4M3, m, k, m));
  LT_CHECK(hipblasLtMatrixLayoutCreate(&B, HIP_R_8F_E4M3, k, n, k));
  LT_CHECK(hipblasLtMatrixLayoutCreate(&C, HIP_R_16F, m, n, m));
  LT_CHECK(hipblasLtMatrixLayoutCreate(&D, HIP_R_16F, m, n, m));
  ...
  if (!algo_count) {
    std::fprintf(stderr, "No FP8 hipBLASLt algorithm for %dx%dx%d\n", m, n, k);
    return 4;
  }
  ...
  // Heuristic ordering is not a throughput guarantee on BW1100. Benchmark all
  // valid candidates and retain the fastest algorithm for this exact shape.
  int best = -1;
  ...
  for (int candidate = 0; candidate < algo_count; ++candidate) {
    ...
    for (int probe = 0; probe < 3; ++probe) {
      if (gemm(results[candidate].algo) != HIPBLAS_STATUS_SUCCESS) break;
    }
  ...
  if (best < 0) {
    std::fprintf(stderr, "No runnable FP8 hipBLASLt algorithm for %dx%dx%d (candidates=%d)\n", m, n, k, algo_count);
    return 4;
  }
  std::fprintf(stderr, "FP8 algo: selected %d/%d (probe %.3f ms, waves %.2f)\n", best, algo_count, best_ms / 3.0f, results[best].wavesCount);
