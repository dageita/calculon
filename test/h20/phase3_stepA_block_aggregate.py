...
def run_block(llm, app, exe, syst, block: str, stage: str, args,
    ...
    if not args.predict_only:
        live, nlive = _live_measure_sum(llm, app, exe, syst, block, stage, args)
        notes.append(nlive)
        sum_iso = live
        iso_complete = live is not None
    ...
    err_deliv_iso = rel_err_pct(block_calc, sum_iso)
    ...
    for stage in stages_seen:
        ...
        for r in sub:
            if r.err_block_vs_calc_pct is None:
                continue
            if r.sum_iso_meas_s is None or 'INCOMPLETE' in r.notes:
                continue
            usable.append(r)
...
def main() -> None:
    ...
    p.add_argument(
        ...
        choices=['kernel_chain', 'absorb'],
        help='kernel_chain: same Phase2 kernels in one timed region (fair H4); '
             'absorb: DeepSeek MLA-absorb+FFN training-path (FP8 Linear)')
    ...
    op_rows = []
    for block in args.blocks:
        op_rows.extend(collect_layer_times(llm, block, stages=stages))
